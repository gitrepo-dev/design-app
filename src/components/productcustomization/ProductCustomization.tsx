...
useEffect(()=>{
  ...
  useEffect(() => {
    // pass the props to product page for local user
  ...
  const handleProduct = (type: string) => {
    dispatch(onAddToCart(customizeProps))
  }
    ...
    <div className="my-4 md:my-8 md:flex items-center">
      <form>
        <div className="flex items-center justify-between mb-5 grid grid-cols-4">
          <label htmlFor="email" className="font-500 mr-2">Caption</label>
          <div className="col-span-3">
            <div className="flex justify-between">
              <input type="text" name="caption" placeholder="Caption" onChange={handleChangeProps} className="w-full focus:outline-0 px-4 rounded border-slate-200 border-2 py-1 mr-2" />
        ...
        <div className="flex items-center justify-between mb-5 grid grid-cols-4">
          <label htmlFor="email" className="font-500 mr-2">Size</label>
          <select name="size" onChange={handleChangeProps} className={`w-full focus:outline-0 px-4 rounded border-slate-200 border-2 py-1 col-span-3 ${selectInputColor && 'input-select'}`} >
        ...
        <div className="flex items-center justify-between mb-5 grid grid-cols-4">
          <label htmlFor="email" className="font-500 mr-2">Colors</label>
          <div className="col-span-3 w-full flex justify-between items-center">
        ...
        <div className="flex items-center justify-between mb-5 grid grid-cols-4">
          <label htmlFor="email" className="font-500 mr-2">Price</label>
          <b>{customizeProps.size > 0 ? `$${customizeProps.size}` : `$0`}</b>
        </div>
        <div className="flex items-center justify-between mb-5 grid grid-cols-4">
          <label htmlFor="email" className="font-500 mr-2 invisible">Print</label>
          <div className="grid grid-cols-2 gap-3 col-span-3">
            <button disabled={customizeProps.size === 0} type="submit" className={`${customizeProps.size === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-700'} rounded-md  py-2 flex items-center justify-center text-white  text-xs `}>
              Print it
